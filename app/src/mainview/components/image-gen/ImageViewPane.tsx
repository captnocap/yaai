...
// IMAGE VIEW PANE
// =============================================================================
// Pane-ready image generation view for the workspace system.
// Displays queue, job monitor, gallery - input comes from GlobalInputHub.
...
export function ImageViewPane({
  ...
  useEffect(() => {
    if (onSettingsChange && imageGen.settings) {
      onSettingsChange({
        defaultModel: imageGen.settings.defaultModel,
        defaultSize: imageGen.settings.defaultSize,
        defaultSteps: imageGen.settings.defaultSteps,
      });
    }
  }, [imageGen.settings, onSettingsChange]);

  // Handle input from GlobalInputHub
  const handleInput = useCallback((input: ViewInput) => {
    if (input.type !== 'image') return;

    // Quick generate with the provided prompt
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Log drawer - no QuickGenerateBar, that's now in GlobalInputHub */}
          <LogDrawer
          ...
          >
            <button
              ...
              style={{
                ...
                padding: '10px',
                border: 'none',
                backgroundColor: mediaTab === 'references' ? 'var(--color-bg)' : 'transparent',
                color: mediaTab === 'references' ? 'var(--color-text)' : 'var(--color-text-secondary)',
                fontSize: '13px',
                fontWeight: 500,
                cursor: 'pointer',
                transition: 'all 0.15s ease',
                borderBottom: mediaTab === 'references' ? '2px solid var(--color-accent)' : '2px solid transparent',
              }}
            >
              References
            </button>
            <button
              ...
              style={{
                ...
                padding: '10px',
                border: 'none',
                backgroundColor: mediaTab === 'gallery' ? 'var(--color-bg)' : 'transparent',
                color: mediaTab === 'gallery' ? 'var(--color-text)' : 'var(--color-text-secondary)',
                fontSize: '13px',
                fontWeight: 500,
                cursor: 'pointer',
                transition: 'all 0.15s ease',
                borderBottom: mediaTab === 'gallery' ? '2px solid var(--color-accent)' : '2px solid transparent',
              }}
            >
              Gallery
            </button>
